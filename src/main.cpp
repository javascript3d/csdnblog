...
void initRest()
{
    ...
    irr_driver = new IrrDriver();//custom class that create IrrLicht device and IrrLich scene manager
...
{
    CommandLine::init(argc, argv);//desktop version has command control, it is not important for web?

    CrashReporting::installHandlers();//CrashReport may be i need one

    srand(( unsigned ) time( 0 ));//For every different seed value used in a call to srand, the pseudo-random number generator can be expected to generate a different succession of results in the subsequent calls to rand.

    try
    {
        std::string s;

        handleCmdLineOutputModifier();//if comand has some word do something, such as for "--help" show the help content
        ...
        if(!UserConfigParams::m_no_start_screen)//do not skip
        {
