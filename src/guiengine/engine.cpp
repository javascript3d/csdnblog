...
namespace GUIEngine
{
    ...
    int getLargeFontHeight()
   {
    ...
    {
        g_env = device_a->getGUIEnvironment();//may be the document of web is one
        g_device = device_a;
        g_driver = driver_a;
    ...
    void addLoadingIcon(irr::video::ITexture* icon)//may be use it to draw progress bar
    {
        if (icon != NULL)
        {
            ...
            g_device->getVideoDriver()
                    ->beginScene(true, true, video::SColor(255,100,101,140));//Applications must call this method before performing any rendering This method can clear the back- and the z-buffer.
            renderLoading(false);
            g_device->getVideoDriver()->endScene();
        }
        else
